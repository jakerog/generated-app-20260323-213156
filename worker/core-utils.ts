...
export abstract class IndexedEntity<S extends { id: string }> extends Entity<S> {
  ...
  static async list<TCtor extends CtorAny>(
    ...
    const { items: ids, next } = await idx.page(normalizedCursor, limit);
    console.log(`[IndexedEntity:${this.indexName}] list: fetched ${ids.length} ids`);
  ...
  static async ensureSeed<TCtor extends CtorAny>(this: HS<TCtor>, env: Env): Promise<void> {
    ...
    console.log(`[IndexedEntity:${this.indexName}] ensureSeed: current index size=${ids.length}, seed size=${seeds?.length ?? 0}`);
    if (ids.length === 0 && seeds && seeds.length > 0) {
      console.log(`[IndexedEntity:${this.indexName}] applying seed data...`);
